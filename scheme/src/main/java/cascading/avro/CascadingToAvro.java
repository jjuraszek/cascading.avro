...
public class CascadingToAvro {
  ...
  public static IndexedRecord fillByName(Schema schema, TupleEntry tupleEntry) {
    GenericRecordBuilder recordBuilder = new GenericRecordBuilder(schema);
    for (int idx = 0; idx < tupleEntry.getFields().size(); idx++) {
      String fieldName = tupleEntry.getFields().get(idx).toString();
      Field avroField = schema.getField(fieldName);
      Object avroValue = toAvro(tupleEntry.getTuple().getObject(idx), avroField.schema());
      recordBuilder.set(fieldName, avroValue);
    }
    return recordBuilder.build();
  }

  protected static Object[] parseTuple(Tuple tuple, Schema writerSchema) {
  ...
  protected static Object[] parseTupleEntry(TupleEntry tupleEntry, Schema writerSchema) {
